...
-- (3) venues can have the same name across cities, but each city only has one venue with the same name. 
--     That is, city and venue_name uniquly define a venue.
-- (4) A concert is defined by their name(not unique) and datetime.
-- (5) A ticket for the same concert cannot be sold twice
